...
Helpers.prototype.write = function write() {
	for (let cn of Object.keys(this.inYarns)) {
		console.log(`Taking out yarn at end: ${cn}.`);
		this.out(`outhook ${cn}`);
	}
	if (OutFile !== "") {
...
function tubeNeedles(dir, bns) {
	...
	return {front, back, sts};
}

Helpers.prototype.start_tube = function start_tube(dir, bns, Carrier) {
	const {front, back, sts} = tubeNeedles(dir, bns);

	//----------------------------------------------------
	//start up a tube with a waste yarn carrier:
	//alternating tuck cast on:
	this.out(`inhook ${StartEndCarrier}`);
	this.inYarns[StartEndCarrier] = true;
	this.out(`x-stitch-number ${CastOnStitchNumber}`);
	sts.forEach(function(dbn, i) {
		if (i%2 == 0) this.knit(dbn[0], dbn[1], StartEndCarrier);
	}, this);
	sts.forEach(function(dbn, i) {
		if (i%2 == 1) this.knit(dbn[0], dbn[1], StartEndCarrier);
	}, this);
	...
	for (let row = 0; row < StartRows; ++row) {
		sts.forEach(function(dbn, i) {
			this.knit(dbn[0], dbn[1], StartEndCarrier);
		}, this);
		if (row == 1) this.out(`releasehook ${StartEndCarrier}`);
	}
	this.out(`outhook ${StartEndCarrier}`);
	delete this.inYarns[StartEndCarrier];
	//----------------------------------------------------


	let toDrop = [];

	{ //bring in yarn and do tuck pattern:
		let release = false;
		...
		const n = parseBedNeedle(bns[0]).needle;
		const initTuck = (d, bn, cs) => {
			this.tuck(d, bn, cs);
			toDrop.push(bn);
		};
	...
	bns.forEach((bn_str) => {
		let d;
		const bn = parseBedNeedle(bn_str)
		if (bn.bed === 'f') {
			d = (dir === 'clockwise' ? '-' : '+');
		} else if (bn.bed === 'b') {
			d = (dir === 'clockwise' ? '+' : '-');
		...
		this.knit(d, bn_str, Carrier);
	});

	//drop any part of tuck pattern that needs dropping:
	...
	toDrop.forEach(function(bn){
		...
		if (idx === -1) {
			this.drop(bn);
			console.log(`Dropping ${bn}`); //DEBUG
		} else {
			console.log(`Not dropping ${bn}`); //DEBUG
		}
	}, this);
};
...
Helpers.prototype.end_tube = function end_tube(dir, bns, Carrier) {
	...
	bns.forEach(function(bn, i) {
		this.knit(d(bn), bn, Carrier);
	}, this);

	//take out the yarn:
	if(this.inYarns[Carrier]){
		this.out("outhook " + Carrier);
		delete this.inYarns[Carrier];
		this.yarnUsedCount[Carrier] = 0;
	...
	//----------------------------------------
	//knit tube with a waste yarn carrier:
